---
slug: minneburg
title_de: Minneburg
title_en: Minneburg
collection: "01"
series_number: NS-01
location: Neckar-Odenwald-Kreis, Baden-Württemberg
coordinates: [49.3891, 9.1667]
season: Spätherbst
viewpoint: Südhang Burgweg, Blick nach Nordosten, Abstand ~80–100 m zum Bergfried
colors: ["#1C2826", "#3D6154", "#6E7B77", "#EDE9DC", "#FAFAF5"]
format: 50×70 cm (2:3)
status: approved
illustration_format: PNG (upscaled raster source master)
source_file: ns-01-minneburg.png
---

## Beschreibung (DE)

Minneburg erscheint hier im warmen Spaetlicht ueber dem Talweg - offene Mauer, Bergfried und Hangwald stehen ruhig gegen einen weiten Himmel. Nicht Verfall ist das Thema, sondern das Beharren des Ortes.

## Description (EN)

Minneburg appears here in late warm light above the valley path - open wall, keep, and wooded slope held against a wide sky. The point is not decay, but the place's persistence.

## Komposition — Entscheidungsprotokoll

- **Dominante Form:** Bergfried, vertikal, leicht rechts der Bildmitte, Spitze bei ~17 % Bildhöhe
- **Silhouette-Basis:** Zweilagiger Wald (Valley Sage Mittelgrund + Charcoal Vordergrund)
- **Himmelszone:** Paper → River Stone (Loess-Streifen als Seitenlicht auf dem Turm, Stärke 18 %)
- **Mauerrest links:** Fragmentarische Wandscheibe als kompositorisches Gegengewicht
- **Typografie:** MINNEBURG in EB Garamond Regular, Paper, unteres Drittel

## Qualitätsgate

- [x] Farbzahl: 5 (≤ 6 ✓)
- [x] Kein Verlauf außerhalb der Himmelszone ✓
- [x] Text-Entfernungs-Test: Bild fühlt sich ohne Titel unvollständig an ✓
- [x] Thumbnail-Test: Hauptelement auf 5×7 cm Bildschirmgröße lesbar ✓
- [x] Keine Anachronismen ✓
- [x] Rasterquelle: als hochaufgeloester PNG-Master fuer 300 dpi Export freigegeben ✓
- [x] Metadaten-Sheet vollständig ✓
- [ ] Web-Vorschau (1200×1680 px WebP) — Export ausstehend
- [ ] Thumbnail (600×840 px WebP) — Export ausstehend

## Empfehlungen

- **Papier:** Hahnemühle German Etching 310 g/m² (Giclée) oder Photo Rag 308 g/m²
- **Rahmung:** Eiche natur, weißer Passepartout 8 cm — Kühle des Motivs nicht durch warmes Holz brechen
- **Alternativ:** Schmales Profil in Schwarz (Stahl oder lackiertes Holz, matt)
