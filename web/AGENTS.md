...
- In Next 16, `unauthorized()` is still part of the experimental auth-interrupt path. If `experimental.authInterrupts` is not enabled in `next.config.ts`, using it in App Router pages can fail at runtime; for `/admin/coa`, prefer `proxy.ts` returning `401` plus `WWW-Authenticate`.
